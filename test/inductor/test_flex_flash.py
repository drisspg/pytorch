...
def create_test_tensors(batch_size=2, num_heads=4, seq_len=512, dim=64, dtype=torch.float16, device="cuda"):
...
def flash_vs_triton(q, k, v, score_mod=None, rtol=5e-3, atol=5e-3):
    ...
    out_flash = compiled_fn(q, k, v, score_mod=score_mod, kernel_options={"disable_flash": False})
    out_no_flash = compiled_fn(q, k, v, score_mod=score_mod, kernel_options={"disable_flash": True})
